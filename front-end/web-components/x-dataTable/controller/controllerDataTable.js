...
class ConstrollerDataTable extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.view.table.addEventListener("sort", (event) =>
      this.onsortrequest(event.detail)
    );
    this.view.paginator.pagesize.addEventListener("change", (event) =>
      this.onpagesizechange(event)
    );

    this.view.filter.addEventListener("keydown", (event) =>
      this.onfilterrequest(event)
    );
    this.view.filter.addEventListener("input", (event) =>
      this.onfilterdefaultrequest(event)
    );

    this.view.paginator.first.addEventListener("click", (event) =>
      this.onfirstpagerequest(event)
    );
    this.view.paginator.previous.addEventListener("click", (event) =>
      this.onpreviouspagerequest(event)
    );
    this.view.paginator.next.addEventListener("click", (event) =>
      this.onnextpagerequest(event)
    );
